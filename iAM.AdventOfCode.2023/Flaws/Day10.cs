...
class Program
{
    private static readonly (int dr, int dc) North = (-1, 0);
    private static readonly (int dr, int dc) South = (1, 0);
    private static readonly (int dr, int dc) West  = (0, -1);
    private static readonly (int dr, int dc) East  = (0, 1);

    // For each pipe character, which directions does it connect to?
    private static readonly Dictionary<char, (int dr, int dc)[]> Connections =
        new()
        {
            ['|'] = new[] { North, South },
            ['-'] = new[] { West, East },
        ...
        char[,] grid, int rows, int cols, int sr, int sc)
    {
        ...
        foreach (var (dr, dc) in dirs)
        {
            ...
            char ch = grid[nr, nc];
            if (Connections.TryGetValue(ch, out var conns))
            {
                foreach (var (cdr, cdc) in conns)
                {
    ...
    private static IEnumerable<(int r, int c)> GetConnectedNeighbors(
        char[,] grid, int rows, int cols, int r, int c,
        List<(int dr, int dc)> sDirs)
    {
        char ch = grid[r, c];
        ...
        else if (!Connections.TryGetValue(ch, out dirs))
        {
            yield break;
        }
        ...
        foreach (var (dr, dc) in dirs)
        {
            ...
            else if (Connections.TryGetValue(nch, out var nDirs))
            {
                foreach (var nd in nDirs)
                {
                    if (nd == (backDr, backDc))
                    {
                        yield return (nr, nc);
